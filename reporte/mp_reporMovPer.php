...
function header()
{
	...
	$this->SetFont('Arial','B',15);
	$this->Cell(55,10,'REPORTE DE MOVIMIENTO',0,0,'R');
	//titulo del reporte
	$this->SetFont('Arial','',10);
...
//-------------------------------------SUB HEADER----------------------------------//
	...
	$pdf->Cell(55,10,"USUARIO",1,0,'C');
	$pdf->Cell(25,10,"AREA",1,0,'C');
	$pdf->Cell(30,10,"FECHA SALIDA",1,0,'L');
	$pdf->Cell(35,10,"FECHA RETORNO",1,0,'L');
	$pdf->Cell(30,10,"UBICACION",1,1,'L');
	*/


//-------------------------------- DATA DE UBICACION ------------------------------------//

	$sql=sql::mp_repMovPerxFil($_POST['mp_mesMov'],$_POST['mp_anMov']);
	$dataMovPer=negocio::getData($sql);

	foreach ($dataMovPer as $data) 
	{

		$sql=sql::mp_detMovShow($data['item']);
		$dataDetMovPer=negocio::getData($sql);

		$sql=sql::mp_detPermiMov($data['item']);
		$dataDetPermi=negocio::getData($sql);
		...
		$pdf->Cell(15,10,$data['item'],1,0,'C');
		$pdf->Cell(55,10,$data['user'],1,0,'C');
		$pdf->Cell(25,10,utf8_decode($data['are']),1,0,'C');
		$pdf->Cell(30,10,$data['fechSali'],1,0,'L');
		$pdf->Cell(35,10,$data['fechRetor'],1,0,'L');
		$pdf->Cell(30,10,$data['desUbi'],1,1,'L');
		$pdf->Ln(5);
		...
		$pdf->SetFont('Arial','B',10);
		$pdf->Cell(70,10,"AUTORIZO",1,0,'C');
		$pdf->Cell(30,10,"AREA",1,0,'C');
		$pdf->Cell(30,10,"CONFIRMACION",1,1,'C');

		foreach ($dataDetPermi as $data) 
		{
			$pdf->SetFont('Arial','',10);
			$pdf->Cell(70,10,utf8_decode($data['user']),1,0,'C');
			$pdf->Cell(30,10,$data['desAprob'],1,0,'C');
			$pdf->MultiCell(30,10,$data['desPrueb'],1);
		}
