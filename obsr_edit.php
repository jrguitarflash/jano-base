...
<div class="box">
	<div class="heading">
		...
		<span class="botone">
			<a href="Javascript:editFormatRecla();" ><img src="images/grabar.png" alt="grabar" width="20px"></a>
	...
	<div class="content">
		<div id="lista_199"> 
					...
					<input class="txtNumRe" type="text" name="numInfor" value="<?php print $numGene; ?>">
					<label class="lblFechRe">Fecha:</label>
					<input class="txtFechRe" id="txtFechRe" type="text" name="txtFechContro" value="<?php print $fechContro; ?>">
				...
				<fieldset class="acciCorre">
					...
					<textarea class="txaAcciCorre" name="txaDesObs"><?php print $desObserv; ?></textarea>
					<label class="titPun">2. Acciones correctivas / preventivas que deben efectuarse</label>
					<textarea class="txaAcciCorre" name="txaEviCorre"><?php print $acciCorre; ?></textarea>
					<label class="titPun">
					...
					<input class="slcResp" id="txtFechLi" type="text" name="txtFechLi" value="<?php print $fechLim; ?>">
				</fieldset>
				<fieldset class="acciCorre">
					<legend>Verificacion</legend>
					<label class="titPun">
						4. Verificacion de Implementacion por el Representante de la Gerencia
						<input type="text" id="txtFechVeri" name="txtFechVeriImp" class="txtFechVeri" value="<?php print $fechVeri; ?>">
					</label>
					<ol type="a" class="veriImp" >
						<li>
							Implementacion satisfactoria
							<?php foreach($dataConfor as $data){ 
								if($data['idConfor']==$idConforImp) {							
							?>
								<input class="yesDatis" type="radio" name="rdImpSati" value="<?php print $data['idConfor']; ?>" checked>
								<?php print $data['valConfor']; ?>
							<?php }else{?>
								<input class="yesDatis" type="radio" name="rdImpSati" value="<?php print $data['idConfor']; ?>">
								<?php print $data['valConfor']; ?>
							<?php }} ?>
						</li>
						<li>
							Fecha acordada para verificacion de efectividad
							<input class="txtFechAcor" id="txtFechAcor" type="text" name="txtFechAcor" value="<?php print $fechAcorVeri; ?>">
						</li>					
					</ol>
					<label class="titPun">
						5. Verificacion de efectividad por el representante de la gerencia
						<input type="text" id="txtFechEfec" name="txtFechVeriEfec" class="txtFechEfec titPunVeriEfe" value="<?php print $fechVeriEfec; ?>">
					</label>
					<ol type="a" class="veriImp" >
						<li>
							Efectividad satisfactoria
							<?php foreach($dataConfor as $data){ 
								if($data['idConfor']==$idConforEfec) {							
							?>
								<input class="yesDatis" type="radio" name="rdEfecSati" value="<?php print $data['idConfor']; ?>" checked>
								<?php print $data['valConfor']; ?>
							<?php }else{?>
								<input class="yesDatis" type="radio" name="rdEfecSati" value="<?php print $data['idConfor']; ?>">
								<?php print $data['valConfor']; ?>
							<?php }} ?>
							<input type="text" id="txtFecEfecSatis" name="txtFecEfecSatis" class="txtFecEfecSatis" value="<?php print $fechEfecSati; ?>">
						</li>				
					</ol>
				</fieldset>
				<fieldset class="acciCorre">
					...
					<textarea class="txaAcciCorre" name="txaEviObje"><?php print $eviObje; ?></textarea>
				</fieldset>
				<fieldset class="acciCorre">
					...
					<div class="panCieNoCon">
						<label class="titPun">
							...
							<input type="text" id="txtFechNoConf" name="txtFechNoConf" class="txtFechNoConf" value="<?php print $fechCie; ?>">
						</label>					
					</div>
					<div class="panCieNoCon">
						<label class="titPun">
							8. Seguimiento en Auditoria Interna: (Follow-up)
							<input type="text" id="txtFechSegAud" name="txtFechSegAud" class="txtFechNoConf" value="<?php print $fechSegui; ?>">
						</label>			
						<ol type="a" class="acEfec" >
						<li>
							AC efectiva
							<?php foreach($dataConfor as $data){ 
								if($data['idConfor']==$idConforAc){							
							?>
								<input class="yesDatis" type="radio" name="rdAc" value="<?php print $data['idConfor']; ?>" checked>
								<?php print $data['valConfor']; ?>
							<?php } else { ?>
								<input class="yesDatis" type="radio" name="rdAc" value="<?php print $data['idConfor']; ?>">
								<?php print $data['valConfor']; ?>
							<?php }} ?>
						</li>
						</ol>	
