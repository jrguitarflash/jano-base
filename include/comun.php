...
require("clases/cuenta.class.php");
...
require_once('clases/programacion.class.php');
//--clases agregadas
...
//
    ...
    if($reg['reg_accion1']=='2'){$cad.="<td class='cAccion' align='center' width='5'><a onclick=\"Accion('U','".$reg['tbl_frm_ancho']."','".$reg['tbl_frm_alto']."','".$form."',".(int)$reg['tbl_padre_id'].",67,".(int)$id.",'".(int)$_REQUEST['menu_id']."','".$reg['tbl_nombre']."','".$array."&coti=adju"."');\"><img src='images/b_edit.png' title='Editar registro'></a></td>";$n+=1;}
		...
		foreach($reg as &$r){
                    ...
                    if($r['tbl_col_orden_lst']>0){
                        if($r['col_control']<>'GRP' && $r['col_control']<>'HR0'){
                            $width=($r['lst_ancho']>0)?" width='".$r['lst_ancho']."px' ":"";
                            $align=" align='center' ";
                            $cad.="<td ".$width.$align." >";
                            ...
                            if($r['tbl_col_orden_lst']>0){
                                ...
                                $campo_orden=($r['tbl_col_campo_orden']>'')?$r['tbl_col_campo_orden']:$r['tabla_col_nombre'];
                                $cad.="<a ".$css_orden." href='#' name='".$campo_orden."' onclick='orden(this.name);return false;'>".$r['tabla_col_rotulo']."</a>";
                            }else{
...
//                    $select.='</select>';
                ...
                $chk=($t['tbl_lst_select']=='1')?1:0;
                
                
                $opcion="<tr><td align='right' height='25' colspan='".($c+$accion+$chk)."'>".$filtro." ".tbl_opcion($id)."</td></tr>";
            }
            $cad.=$colspan."</tr>";
	}
	return $opcion.$cad;
...
function tbl_contenido($id){
                    ...
                    if($r['tbl_col_ctrl_filtro']){
                        ...
                        switch($r['col_control']){
                            case 'MES':
                                ...
                                $filtro=$tbl_contenido_REQUEST['mes_'.$r['tabla_col_nombre']];
                                break;
                            case 'AXO':
    ...
    */
	...
	$_SESSION['operador']['flow']=$flow;
        ...
        switch($t['tbl_filtro_conttbl_contenidorol']){
            case 'MES':
            case 'FEC':
            case 'AXO':
        ...
        /*
            if($_SESSION['SIS'][5]==1 and $_SESSION['SIS'][3]==6 
                and ($_GET['tbl_id']==67 or $_GET['tbl_id']==205 or $_GET['tbl_id']==206 or $_GET['tbl_id']==199) and $_GET['menu']=='personas')
            {
                $condicion=tbl_param($t['tbl_sql_cond'].' and operador_id_update=SIS2');
            }
            else
            {
        */
        ...
        //cambiar order de propiedades
        if($t['tbl_nombre']=='prod_propiedad')
        {
	    ...
	    $sql="SELECT SQL_CALC_FOUND_ROWS *,".implode($campos,',')." FROM ".$t['tbl_nombre']." WHERE ".$where.$order.$limite;
        $result=mysql_query($sql);
	...
	if($result){
		...
		while($row=mysql_fetch_array($result,1)){
                    ...
                    $cad.="<tr ".$css.">";
                    if($t['tbl_lst_select']=='1'){
                        $cad.="<td class='cAccion' width='10' align='center'><input type='checkbox' name='selected[]' value='".$row[$t['tbl_col_pk']]."'/></td>";
                    }
                    $cad.=$fila_contenido;
                    $cad.=tbl_accion('C',$id,$t['lst_cpo_form'],$row[$t['tbl_col_pk']],$param);
                    #$cad.="<td><a onclick=\"Accion('U','".$reg['tbl_frm_ancho']."','".$reg['tbl_frm_alto']."','".$form."',".(int)$reg['tbl_padre_id'].",".(int)$tbl_id.",".(int)$id.",'".(int)$_REQUEST['menu_id']."','".$reg['tbl_nombre']."','".$array."');\"><img src='images/b_edit.png' title='Editar registro'></a></td>";
                    $cad.="</tr>";
		}
	}
...
function tbl_paginacion($formato,$tbl_id,$page,$num_rows,$rows_per_page,$total_rows=0){
    ...
    switch($formato){
        case 1: // Muestra enlaces de paginacion
            ...
            $link[]='<b>'.($offset+1).' - '.($o+1).' de '.($num_rows).'</b>';
            ...
            if($num_rows>$rows_per_page){
                ...
                $links='<b>'.($offset+1).' - '.($o+1).' de '.($num_rows).'</b>';
            }
...
function tbl_form_opcion($tbl_id=0){
	...
	if($reg['frm_acc_grabar']=='1'){$cad[]="<a id='btnSave'><img src='images/grabar.png' width='20' height='20' title='Grabar'></a>";}
        if($reg['frm_acc_duplicar']=='1'){$cad[]="<a id='btnDuplicar' style='display:none' ><img src='images/duplicar.png' title='Duplicarr'></a>";}
	if($reg['frm_acc_eliminar']=='1'){$cad[]="<a href='#'><img src='images/eliminar.png' title='Eliminar'></a>";}
        ...
        if($_SESSION['SIS'][7]==0)
        {
        	if($tbl_id=="67" && $_REQUEST['coti']=="adju" ) { $tbl_id="205"; }
            elseif($_REQUEST['coti']=="proye"){ $_REQUEST['menu_id']=29; $reg['lst_cpo_list']='cot_proyeCot'; }
    ...
    switch($accion){
        case 'F': // Formulario
            if($reg['reg_nombre']>''){
        		...
        		if(isset($_GET['coti']) and $_GET['coti']=='adju') {
        			echo "<h1>Cotizacion - Adjudicada: <span>".$valor."</span></h1>";	
        		}
                elseif(isset($_GET['coti']) and $_GET['coti']=='proye')
                {
                    echo "<h1>Cotizacion Activa: <span>".$valor."</span></h1>";
                }
        		else{
    ...
    switch($campo['col_control']){
        ...
        case 'LST':
            ...
            /*
                ...
                and ($_GET['tbl_id']==67 or $_GET['tbl_id']==205 or $_GET['tbl_id']==206 or $_GET['tbl_id']==199) and $_GET['menu']=='personas')
                {
                    $objeto=explode("|",$campo['fuente_tbl'].' and trabajador_id=SIS2');
                }
                else
                {
            */
        ...
        case 'ADJ':
            ...
            $ctrl='<div class="file"><span id="file_'.$campo['tabla_col_nombre'].'">'.$file.'</span><a title="View file" class="ui-icon ui-icon-zoomin"></a></div>
...
.cotCond2 tr td{
    ...
    $cliente=$reg['cliente'].'<br><font size="8">'.$reg['cli_direccion'].'<br>Tel.: '.$reg['cli_telef'].'</font>';

    if($_SESSION['SIS'][5]==1) //Cotizaciones formato Electrowerke
    {
        $str.='<table width="100%" class="cotGen" cellpadding="0px"  cellspacing="0px" border="0px">
              <tr>
                <td bgcolor="#CCCCCC" colspan="5" align="center" valign="center" >
                    COTIZACION NRO '.$reg['cot_nro'].'
                </td>
              </tr>
              <tr>
                ...
                <td align="right" width="15%">Referencia:</td>
                <td align="center" width="25%" >'.$reg['cot_referencia'].'&nbsp;</td>
              </tr>
              <tr>
                <td>ATENCION </td>
                <td >:</td>
                <td colspan="3">'.$reg['contact_nombre'].'</td>
              </tr>
              <tr>
                  <td>PROYECTO </td>
                  <td >:</td>
                  <td>'.$reg['proy_nombre'].'&nbsp;</td>
    ...
    else //Cotizacion Formato EltroTec
    {
        $str.='<table width="100%" class="cotGen2" cellpadding="5px"  cellspacing="0px" border="0px">
              <tr>
                <td bordercolor="#CCCCCC" colspan="5" align="center" valign="center" >
                    <strong>COTIZACION NRO '.$reg['cot_nro'].'</strong>
                </td>
              </tr>
              <tr>
                ...
                <td width="5%">:</td>
                <td width="35%">'.$cliente.'</td>
                <td align="right" width="15%"><strong>Referencia:</strong></td>
                <td align="center" width="25%" >'.$reg['cot_referencia'].'&nbsp;</td>
              </tr>
              <tr>
    ...
    if($_SESSION['SIS'][5]==1) //Formato detalle Electrowerke
    {

            $str.='<table width="100%" border="1" cellpadding="3" class="detCot" >
                <tr bgcolor="#CCCCCC">
         ...
         foreach($items as $item){
            $c++;
            $str.=($item['cot_det_titulo']>'')?'<tr bgcolor="#999999"><td width="6%">&nbsp;</td><td colspan="5"><b>'.$item['cot_det_titulo'].'</b></td></tr>':'';
            $i=($item['cot_det_orden']>'0')?$item['cot_det_orden']:$c;
            $str.='<tr>
            ...
            $propiedad=($propiedad>'')?'<table class="list">'.$propiedad.'</table>':'';
             ...
             if($item['marca_nombre']>''){
                 $marca='<tr><td width="25%">Marca:</td><td width="75%">'.$item['marca_nombre'].'</td></tr>
    ...
    else //Formato detalle EltroTec
    {
                $str.='<table width="100%" border="0" cellpadding="3" class="detCot2">
                    <tr bordercolor="#CCCCCC">
                        <td width="6%" align="center"><strong>ITEM</strong></td>
                        <td width="50%" align="center"><strong>DESCRIPCION</strong></td>               
                                    
                        <td align="center" width="10%"><strong>CANT.</strong></td>
                        <td align="center" width="6%"><strong>MON.</strong></td>
                        <td align="center" width="14%"><strong>UNIT.</strong></td>
                        <td align="center" width="14%"><strong>TOTAL</strong></td>
                    </tr>';
             $items=cotizacion::edit("D",$reg['cotizacion_id']);
             $search=array("<",">");
             $replace=array("","");
     ...
     if($_SESSION['SIS'][5]==1) //Condiciones Electrowerke
     {
         $str.='<table width="100%" class="cotCond" >
        <tr>
        ...
        <tr><td>Validez</td><td>:</td><td>'.nl2br($valiCond).'&nbsp;</td></tr>
        </table></body></html>';
     }
     else //Condiciones EltroTec
     {
            $str.='<table width="100%" class="cotCond2">
            <tr>
                <td colspan="1" align="right" >
                    <u><strong>CONDICIONES COMERCIALES</strong></u>:<br>
                </td>
            </tr>
            ...
            <tr>
                <td >'.nl2br($reg['cot_cond_precios']).'&nbsp;</td>
            </tr>
            <tr>
                <td ><strong>Plazo de Entrega:</strong></td>
            </tr>
            <tr>
                <td>'.nl2br($reg['cot_cond_plazo_ent']).'&nbsp;</td>
            </tr>
            <tr>
                <td ><strong>Lugar de Entrega:</strong></td>
            </tr>
            <tr>
                <td>'.nl2br($reg['cot_cond_lugar_ent']).'&nbsp;</td>
            </tr>
            <tr>
                <td ><strong>Forma de Pago</strong></td>
            </tr>
            <tr>
                <td>'.nl2br($reg['cot_cond_forma_pago']).'&nbsp;</td>
            </tr>
            <tr>
                <td ><strong>Garantía:</strong></td>
            </tr>
            <tr>
                <td>'.nl2br($reg['cot_cond_garantia']).'&nbsp;</td>
            </tr>
            <tr>
                <td ><strong>Validez:</strong></td>
            </tr>
            <tr>
                <td>'.nl2br($valiCond).'&nbsp;</td>
            </tr>
            </table></body></html>';
     }
...
function ProformaSalida($proforma_id=0,$empresa_id=0){
    ...
    {
        $str=file_get_contents('plantillas/pro_formato_et.html');
    }
...
function OCLSalida($compras_id=0,$empresa_id=0){
    ...
    foreach($items as $item){
        ...
        $total+=$sub;
        $descripcion=str_replace(chr(13),'<br>',$item['prod_descripcion']);
        $detalle.='<tr>
                <td align="center">'.$c.'</td>
                <td><b>'.$item['producto_nombre'].'</b><br>'.$descripcion.'</td>
                <td align="center">&nbsp;'.$item['prod_cantidad'].'</td>
                <td align="right">'.number_format($item['prod_precio_venta'],2,'.',',').'&nbsp;</td>
                <td align="right">'.number_format($sub,2,'.',',').'&nbsp;</td>
                </tr>';
     }
     $igv=($reg['comp_igv']>0)?$reg['comp_igv']/100:0;
     $igv=($total*$igv);
     $total=$total+$igv;
     if($igv>0){
        $detalle.='<tr><td colspan="4" align="right">IGV '.$reg['moneda'].'</td><td align="right">'.number_format($igv,2,'.',',').'</td></tr>';
     }
     $detalle.='<tr><td colspan="4" align="right">Total '.$reg['moneda'].'</td><td align="right">'.number_format($total,2,'.',',').'</td></tr>';
     $detalle.='</tbody></table>';
     
     $condiciones='<table><tbody><tr><td colspan="2"><b>&nbsp;Indicaciones:</b></td></tr>';
     $condiciones.=($reg['comp_cond_precios']>'')?'<tr><td width="25%">&nbsp;Precios</td><td width="75%">'.$reg['comp_cond_precios'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_plazo_ent']>'')?'<tr><td>&nbsp;Plazo Entrega</td><td>'.$reg['comp_cond_plazo_ent'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_lugar_ent']>'')?'<tr><td>&nbsp;Lugar Entrega</td><td>'.$reg['comp_cond_lugar_ent'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_forma_pago']>'')?'<tr><td>&nbsp;Forma Pago</td><td>'.$reg['comp_cond_forma_pago'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_garantia']>'')?'<tr><td>&nbsp;Garantia</td><td>'.$reg['comp_cond_garantia'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_validez']>'')?'<tr><td>&nbsp;Validez</td><td>'.$reg['comp_cond_validez'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_penalidad']>'')?'<tr><td>&nbsp;Penalidad</td><td>'.$reg['comp_cond_penalidad'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_carta_fianza']>'')?'<tr><td>&nbsp;Canta Fianza</td><td>'.$reg['comp_cond_carta_fianza'].'</td></tr>':'';
     $condiciones.='</tbody></table>'; 
     $firma=($reg['firma']>'')?'<img src="'.$reg['firma'].'" width="200" height="95" border="0" />':'';
     $marks=str_replace(chr(13),'<br>',$reg['comp_marks']);     
     $proveedor=$reg['proveedor'].'<br><font size="8">'.$reg['prov_direccion'].'<br>Tel.: '.$reg['prov_telef'].'</font>';
     $empresa=$reg['empresa'].'<br><font size="8">'.$reg['emp_direccion'].'<br>Tel.: '.$reg['emp_telef'].'</font>';
     $search=array('[FIRMA]','[NRO]','[PROVEEDOR]','[EMPRESA]','[PROYECTO]','[PC]','[REFERENCIA]','[FECHA]','[DETALLE]','[CONDICIONES]','[MARCA]');
     $replace=array($firma,$reg['comp_nro'],$proveedor,$empresa,$reg['proyecto'],'PC-'.str_pad($reg['pcId'],4,'0',STR_PAD_LEFT),$reg['oc_nro'],$reg['fecha'],$detalle,$condiciones,$marks);
...
function OCSalida($compras_id=0,$empresa_id=0){
    ...
    $search=array("<",">");
    $replace=array("","");
    if($reg['compra_tipo_id']==1){ // Compra Nacional
        $lbl_fecha='FECHA';
    ...
    $proveedor=$reg['proveedor'].'<br><font size="8">'.$reg['prov_direccion'].'<br>Tel.: '.$reg['prov_telef'].'</font>';
    $invoice=$reg['invoice'].'<br><font size="8">'.$reg['inv_direccion'].'<br>Tel.: '.$reg['inv_telef'].'</font>';
    $delivery=$reg['delivery'].'<br><font size="8">'.$reg['del_direccion'].'<br>Tel.: '.$reg['del_telef'].'</font>';
    
    $items=compras::edit("D",$reg['compras_id']);
    $detalle='<table border="1" cellpadding="3" >';

    //EDICION DE CABECERA ITEMS
    if($_SESSION['SIS'][5]==1)
    {
        $detalle.='<thead>
        <tr bgcolor="#CCCCCC">
    ...
    else
    {
        $detalle.='<thead>
        <tr bgcolor="white">
        <td width="10%" align="center"><strong>'.$lbl_id.'</strong></td>
        <td width="50%" align="center"><strong>'.$lbl_prod_nombre.'</strong></td>
        <td width="10%" align="center"><strong>'.$lbl_cantidad.'</strong></td>
        <td width="15%" align="center"><strong>'.$lbl_precio.'</strong></td>
        <td width="15%" align="center"><strong>'.$lbl_st.'</strong></td>
        </tr></thead><tbody>';
    }
    ...
    foreach($items as $item){
     ...
     switch($reg['tipo_precio_id']){         
         case 3: // CIF
             $cif=$reg['comp_flete']+$total;
             if($reg['comp_flete']>0){
                 $detalle.='<tr><td colspan="4" align="right">Total FOB '.$moneda.' </td><td align="right">'.number_format($total,2,'.',',').'</td></tr>';
                 $detalle.='<tr><td colspan="4" align="right">Insurance+Freight</td><td align="right">'.number_format($reg['comp_flete'],2,'.',',').'</td></tr>';
                 $detalle.='<tr><td colspan="4" align="right">Total CIF '.$moneda.' </td><td align="right">'.number_format($cif,2,'.',',').'</td></tr>';
             }else{
                 $detalle.='<tr><td colspan="4" align="right">Total CIF '.$moneda.' </td><td align="right">'.number_format($total,2,'.',',').'</td></tr>';
             }                 
             break;
         default:
             $detalle.='<tr><td colspan="4" align="right">Total Price '.$moneda.' </td><td align="right">'.number_format($total,2,'.',',').'</td></tr>';
             break;
     }     
     $detalle.='</tbody></table>';
     //$condiciones='<table width="100%" border="0">';
     $condiciones.=($reg['comp_cond_precios']>'')?'<tr><td width="20%">'.$c_precios.'</td><td width="80%">'.$reg['comp_cond_precios'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_plazo_ent']>'')?'<tr><td width="20%">'.$c_plazo.'</td><td width="80%">'.$reg['comp_cond_plazo_ent'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_lugar_ent']>'')?'<tr><td width="20%">'.$c_lugar.'</td><td width="80%">'.$reg['comp_cond_lugar_ent'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_forma_pago']>'')?'<tr><td width="20%">'.$c_pago.'</td><td width="80%">'.$reg['comp_cond_forma_pago'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_garantia']>'')?'<tr><td width="20%">'.$c_garantia.'</td><td width="80%">'.$reg['comp_cond_garantia'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_validez']>'')?'<tr><td width="20%">'.$c_validez.'</td><td width="80%">'.$reg['comp_cond_validez'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_penalidad']>'')?'<tr><td width="20%">'.$c_penalidad.'</td><td width="80%" >'.$reg['comp_cond_penalidad'].'</td></tr>':'';
     $condiciones.=($reg['comp_cond_carta_fianza']>'')?'<tr><td width="20%">'.$c_carta.'</td><td width="80%">'.$reg['comp_cond_carta_fianza'].'</td></tr>':'';
     
    $condiciones=($condiciones>'')?'<table width="100%" border="0">'.$condiciones.'</table>':'';
     //$condiciones.='</table>';
    $firma=($reg['firma']>'')?'<img src="'.$reg['firma'].'" width="200" height="95" border="0" />':'';
    $marks=str_replace(chr(13),'<br>',$reg['comp_marks']);
    $notes=str_replace(chr(13),'<br>',$reg['comp_note']);
    $notes=($notes>'')?'<tr><td class="condiciones"><b>Notes:</b><br><small>'.$notes.'</small></td></tr>':'';
    $responsable=$reg['responsable']."<br>".$reg['responsable2'];
    $search=array('[FIRMA]','[SUPP]','[MARKS]','[NOTES]','[INVOICE]','[DELIVERY]','[EMPRESA]','[NRO]','[TITULO]','[PROVEEDOR]','[CLIENTE]','[CONTACTO]','[PROYECTO]','[PC]','[OPERADOR]','[FECHA]','[DETALLE]','[CONDICIONES]','[FIRMA]','[REFERENCIA]',
                  '[LBL_PROVEEDOR]','[LBL_CLIENTE]','[LBL_CONTACTO]','[LBL_PROYECTO]','[LBL_OPERADOR]','[LBL_FECHA]','[LBL_FIRMA]','[LBL_REFERENCIA]');
    $replace=array($firma,$reg['proveedor'],$marks,$notes,$invoice,$delivery,$reg['empresa'],$reg['comp_nro'],$lbl_titulo,$proveedor,'[CLIENTE]',$reg['prov_contacto'],$reg['proyecto'],'PC-'.str_pad($reg['pcId'],4,'0',STR_PAD_LEFT),$responsable,$reg['fecha'],$detalle,$condiciones,'[FIRMA]',$reg['oc_nro'],
    ...
    {
        $str=file_get_contents('plantillas/oc_formato2.html');
    }
...
function prod_cat_ddl($id){
    $reg=producto::prod_categoria($id);
    
    $str='<option value=""></option>';
    foreach ($reg as $value){
        $str.='<option value="'.$value['prod_categoria_id'].'" >'.$value['prod_cat_nombre'].'</option>';
    }
    return $str;
}
...
function EnviarCorreo($tipo='',$id=0){
    ...
    switch ($tipo){
        ...
        case 'OCL':
            $reg=oc::edit('S',$id);
            $para=$reg['correo_contacto'];
            $asunto="PURCHASE ORDER Nº ".$reg['oc_nro'];
            $de=$reg['correo_atiende'];
...
function cotizacion_lista($empresa_id=0){
    ...
    if(is_array($reg)){
        ...
        foreach ($reg as $value){
            ...
            $url="index.php?menu_id=29&menu=persona_form&tbl_id=".$tbl."&id=".$value['cotizacion_id']."&a=U&cotizacion_id=".$value['cotizacion_id']."&cli_id=".$value['cliente_id'];
            $str.="<tr>
...
function cuenta_lista_reporte(){
    ...
    foreach($reg as $item){
        $mov=cuenta::movimiento($item['cuenta_id']);
        $str.='<tr><td>'.$item['banco'].' '.$item['moneda'].'</td>';
        $str.='<td colspan="2">Cta. '.$item['cuenta_nro'].'</td></tr>';
        $str.='<tr><td colspan="2" align="right">Saldo Inicial</td><td></td></tr>';
        $saldo='';
        foreach ($mov as $value) {
            $str.='<tr><td>Decripcion</td>';
            $str.='<td align="center">'.$value['fecha'].'</td>';
            switch($value['movimiento_id']){
                case 1: // Ingreso
                    $str.='<td align="right">'.$value['cta_mov_monto'].'</td></tr>';
                    break;
                case 2: // Salida
                    $str.='<td align="right">-'.$value['cta_mov_monto'].'</td></tr>';
                    break;
            }
            $saldo=$value['cta_mov_saldo'];
        }
        $str.='<tr><td colspan="2" align="right">Sub Total</td><td></td></tr>';
        $str.='<tr><td colspan="2" align="right">Saldo Disponible</td><td align="right">'.$saldo.'</td></tr>';
    }    
    $str.="</tbody></table>";
    return $str;
}
...
function compras_reporte($ano=0,$mes=0){
    $reg=compras::lista_reporte($ano,$mes);
    $str='<table class="list" border="1" width="100%">
          <thead><tr align="center" bgcolor="#CCCCCC">
          <td width="5%">Id</td>';
    foreach ($reg as $item){
        $str.='<td align="center">'.$item['proveedor'].'</td>';
        $total+=$item['total'];
    }
    $str.='</tr></thead>';
    $str.='<tbody><tr><td>Totales:</td>';
    foreach ($reg as $item){
        $str.='<td align="center">$ '.number_format($item['total'],2,'.',',').'</td>';
        //$st=$item['total']*100/$total;
    }
    $str.='</tr><tr><td>%</td>';
    foreach ($reg as $item){
        $st=$item['total']*100/$total;
        $str.='<td align="center">'.round($st,2).'</td>';
...
function programacion_lista(){    
    $reg=programacion::edit('L',0);
    $str="<table class='list'>".cabecera(false,'Proceso,Referencia,Tarea,Responsable,Fecha,Estado',1);
    $str.="<tbody>";
    foreach($reg as $value){
        $str.='<tr>
            <td class="center">'.$value['proceso'].'</td>
            <td>'.$value['prog_referencia'].'</td>
            <td class="left">'.$value['prog_tarea_nombre'].'</td>
            <td >'.$value['para_nombre'].'</td>
            <td >'.$value['prog_fec_ini'].'</td>
            <td class="center">'.$value['estado'].'</td>
            <td class="center"><a onclick="Alertas(0,'.$value['programacion_id'].')"><img title="Finalizar" src="images/success.png"></a></td>
            </tr>';
    }
    $str.="</tbody></table>";
    return $str;     
...
function alerta_mail(){
    $reg=programacion::edit('L',0);
    foreach($reg as $value){
        if($value['prog_aviso']=='0'){
            $de=array("mail"=>$value['de_mail'],"nombre"=>$value['de_nombre']);
            $para=array("mail"=>$value['para_mail'],"nombre"=>$value['para_nombre']);
            $files=array();
            if(send_mail($de,$para,$value['prog_tarea_nombre'],$value['prog_descripcion'], $files,$cc='',$cco='')){
                programacion::edit('A',$value['programacion_id']);
            }
        }
    }
}
...
function alertas(){
    $reg=programacion::edit('C',0);
    if($reg>0){
        $str='<li id="store"><a class="top" href="index.php"><font color="#FFFF00">Alertas (<b>'.$reg.'</b>)</font></a></li>';
    }
    return $str;
    //echo '<li id="store"><a class="top" target="_blank" href="ayuda.php?id='.$_GET['tbl_id'].'">Ayuda</a></li>';
}

function send_mail($de=array(),$para=array(), $asunto='', $mensaje='', $files=array(),$cc='',$cco=''){
    $mail=new PHPMailer(); // defaults to using php "mail()"
    $mail->SetFrom($de['mail'],$de['nombre']);
    //$mail->AddReplyTo("andy.r3@gmail.com","First Last");
    $mail->AddAddress($para['mail'],$para['nombre']);
    $mail->Subject=$asunto;
...
function compras_tipo_ddl($id=0)
{    
    $reg=compras::compras_tipo_ddl();
    $str='<option value=""></option>';
    foreach ($reg as $value){
        $sel=($value['compra_tipo_id']==$id)?' selected ':'';
        $str.='<option value="'.$value['compra_tipo_id'].'" '.$sel.'>'.$value['compras_tipo_nombre'].'</option>';
    }
    return $str;
}

function getPerfil($id,$val)
{
	$sql=sql::getPerfil($id);
	$perfil=negocio::getVal($sql,$val);
	return $perfil;	
}

function setEmail($des,$acci,$acciReli,$idConfir,$tip,$fech,$cli,$contac,$sr,$sr2,$emailRecep,$emailResp,$emailResp2,$fileAdju)
{
	$mail = new PHPMailer();
	$acciArr=Array();
	$acciReliArr=Array();
	...
	for($i=0;$i<count($acciArr);$i++) {
		$acciCad=$acciCad."<br/>".$acciArr[$i];
	}

	for($i=0;$i<count($acciReliArr);$i++) {
		$acciReliCad=$acciReliCad."<br/>".$acciReliArr[$i];
	}

	#$body             = file_get_contents('contents.html');
	$body  = "<meta http-equiv='Content-Type' content='text/html; charset=ISO-8859-1' />";
	$body .= ("<p>Hola Sr. ".$sr.",".$sr2." usted a sido seleccionado para dar solucion al reclamo N° RE-".str_pad($idConfir,5,'0',STR_PAD_LEFT)." es por ello le hacemos "); 
	$body .=	('llegar los links de confirmacion que solo puede acceder a esta opcion en las instalaciones de electro werke</p>');
	$body .= ('<br/><br/>');
	$body .= ("<a href='http://192.168.1.3/dev/erp-dev/confirm.php?id=".$idConfir."&tip=acep&corre1=".base64_encode($emailRecep)."&corre2=".base64_encode($emailResp)."&corre3=".base64_encode($emailResp2)."&sr=".base64_encode($sr)."&sr2=".base64_encode($sr2)."'>");
	$body .=	('Aceptar asignacion reclamo</a>');
	$body .= ('<br/>');
	$body .=	("<a href='http://192.168.1.3/dev/erp-dev/confirm-close.php?id=".$idConfir."&tip=recha&corre1=".base64_encode($emailRecep)."&corre2=".base64_encode($emailResp)."&corre3=".base64_encode($emailResp2)."&sr=".base64_encode($sr)."&sr2=".base64_encode($sr2)."'>");
	$body .=	('Rechazar asignacion reclamo</a>');
	$body .= ('<br/>');
	$body .=	("<a href='http://192.168.1.3/dev/erp-dev/confirm-close.php?id=".$idConfir."&tip=soli&corre1=".base64_encode($emailRecep)."&corre2=".base64_encode($emailResp)."&corre3=".base64_encode($emailResp2)."&sr=".base64_encode($sr)."&sr2=".base64_encode($sr2)."'>");
	$body .=	('Solicitar cierre de reclamo</a>');
	$body .= ('<br/>');
	$body .= ('<br/>');
	$body .= ('<br/>');
	$body .= ('<p><strong>Descripcion: </strong>'.$des.'</p>');
	$body .= ('<br/>');
	$body .= ('<p><strong>Acciones ordenadas: </strong><br/>'.$acciCad.'</p>');
	$body .= ('<br/>');
	$body .= ('<p><strong>Acciones realizadas: </strong><br/>'.$acciReliCad.'</p>');
	$body .= ('<br/>');
	$body .= ('<strong>Detalle Reclamo: </strong>');
	$body .= ('<br/>');
	$body .= ('Tipo: '.$tip);
	$body .= ('<br/>');
	$body .= ('Fecha: '.$fech);
	...
	$mail->Subject    = "Email de Confirmacion para solucion de reclamo";
	
	$mail->AltBody    = "Por favor recuerde confirmar o rechazar las acciones del reclamo"; // optional, comment out and test
    ...
    $mail->AddAddress($address3, $sr);
	
    /*
	$mail->AddAttachment($fileAdju[0]);
	$mail->AddAttachment($fileAdju[1]);
	$mail->AddAttachment($fileAdju[2]);
    */

    for($i=0;$i<count($fileAdju);$i++)
    {
        $mail->AddAttachment($fileAdju[$i]);
    }      
...
function setEmailAcep($idConfir,$sr,$sr2,$emailRecep,$emailResp,$emailResp2,$tare,$asun,$valAdju)
{
	$mail = new PHPMailer();
	...
	if($tare=='acep') {
		$body  = "<meta http-equiv='Content-Type' content='text/html; charset=ISO-8859-1' />";
		$body .= ("<p>El reclamo N° RE-".str_pad($idConfir,5,'0',STR_PAD_LEFT)." asignado al  Sr. ".$sr.", ".$sr2." fue aceptado "); 
		$body .=	('para la pronta solucion</p>');
		$confir="Aceptacion";
	}
	else if($tare=='recha') {
		$body  = "<meta http-equiv='Content-Type' content='text/html; charset=ISO-8859-1' />";
		$body .= ("<p>El reclamo N° RE-".str_pad($idConfir,5,'0',STR_PAD_LEFT)." asignado al  Sr. ".$sr.", ".$sr2." fue rechazado "); 
		$body .=	('para la pronta solucion</p>');
		$body .= ("<br/><br/>");
		...
		$body  = "<meta http-equiv='Content-Type' content='text/html; charset=ISO-8859-1' />";
		$body .= ("<p>El reclamo N° RE-".str_pad($idConfir,5,'0',STR_PAD_LEFT)." asignado al  Sr. ".$sr.", ".$sr2." fue solicitada a cierre "); 
		$body .=	('para la pronta solucion</p>');
		$body .= ("<br/><br/>");
		...
		$body  = "<meta http-equiv='Content-Type' content='text/html; charset=ISO-8859-1' />";
		$body .= ("<p>El reclamo N° RE-".str_pad($idConfir,5,'0',STR_PAD_LEFT)." asignado al  Sr. ".$sr.", ".$sr2." fue solucionado "); 
...
function send_mail_coti($de,$para=array(), $asunto='', $mensaje='', $files=array(),$cc='',$cco='')
{
    $mail=new PHPMailer(); // defaults to using php "mail()"
