...
foreach ($dataPeri as $data) 
{
		...
		$sql=negocio::evaBusPeri($_POST['valAr'],$data['vaca_perioAn_id'],$_POST['valTrab']);
		...
		<table class="list" >
			...
			<tbody>
					...
					<?php
						$totTom=0;
						$totPen=0;
						$sql=negocio::evaFilPeriPru($data1['vaca_trabId'],$data['vaca_perioAn_id'],$_POST['valAr']);
						$dataPeriTrab=negocio::getData($sql);
						$sql=sql::vaca_getVacaIni($data1['vaca_trabId']);
						$valFechIni=negocio::getVal($sql,'fechModi');
						$sql=sql::vaca_getForCal($data1['vaca_trabId'],$data['vaca_perioAn_id']);
